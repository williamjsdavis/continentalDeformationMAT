...
% Colourbars
cbar = redblue;close all % Set colourbar to red/blue
...
% Properties
eij_dot = strain_rate(Ux_2,Uy_2,h,1); % Strain rate tensor
...
eij_dot_dim = strain_rate(Ux_2,Uy_2,h,1E-6/(365.25*24*60*60)*u0/L); % Strain rate tensor [s^-1]
...
tij_dim = strain_rate(Ux_2,Uy_2,h,1*g*pc*L*(1-pc/pm)/Ar*E_dot.^(1/n-1)); % Deviatoric stress tensor [MPa]
...
hold on
contourf(L*X,L*Y,u0*sqrt(Ux_2.^2+Uy_2.^2),'LineColor','none');
quiver(L*X(vector,vector),L*Y(vector,vector),...
    u0*Ux_2(vector,vector)./sqrt(Ux_2(vector,vector).^2+Uy_2(vector,vector).^2),...
    u0*Uy_2(vector,vector)./sqrt(Ux_2(vector,vector).^2+Uy_2(vector,vector).^2),'LineWidth',1.5,'Color','k')
